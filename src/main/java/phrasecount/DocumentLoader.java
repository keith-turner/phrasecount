...
public class DocumentLoader implements Loader {
  ...
  private void setRefCount(TypedTransactionBase tx, String hash, int rc) {
    ...
    // TODO want to trigger checking for indexing in all cases when the ref count transitions to 0
    // or 1, except when it transitions from 2 to 1.... the
    // following transitions should trigger a check for indexing
    // null->1
    // 0->1
    // 1->0
    if (rc == 0 || rc == 1)
      tx.mutate().row("doc:" + hash).col(INDEX_CHECK_COL).set(); // setting this triggers the phrase
                                                                 // counting observer
  }
